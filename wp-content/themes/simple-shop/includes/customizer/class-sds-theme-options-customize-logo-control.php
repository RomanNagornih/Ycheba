		...
		<div class="customize-image-picker customize-sds-theme-options-logo-upload">
			...
			<p>
				<?php
					...
					printf( __( 'Upload a logo to to replace the site name. Recommended dimensions: %1$s.', 'simple-shop' ), $sds_logo_dimensions );
				?>
			</p>

			<strong><?php _e( 'Current Logo:', 'simple-shop' ); ?></strong>
			<div class="sds-theme-options-preview sds-theme-options-logo-preview">
				...
				?>
						<div class="description"><?php _e( 'No logo selected.', 'simple-shop' ); ?></div>
				<?php endif; ?>
			</div>
			...
			<input type="submit" id="sds_theme_options_logo_attach" class="button-primary sds-theme-options-upload" name="sds_theme_options_logo_attach"  value="<?php esc_attr_e( 'Choose Logo', 'simple-shop' ); ?>" data-media-title="Choose A Logo" data-media-button-text="Use As Logo" />
			<?php submit_button( __( 'Remove Logo', 'simple-shop' ), array( 'secondary', 'button-remove-logo' ), 'sds_theme_options[remove-logo]', false, ( ! isset( $sds_theme_options['logo_attachment_id'] ) || empty( $sds_theme_options['logo_attachment_id'] ) ) ? array( 'disabled' => 'disabled', 'data-init-empty' => 'true' ) : false ); ?>
		</div>
	<?php
	}
}